...
import sponsorService from "../services/sponsor/clubSponsor";
import { objectId,  club_sponsor } from "../../types/type";
type obj = club_sponsor & objectId
const initialState: {
  game: club_sponsor | null,
  games: club_sponsor[],
  loading: boolean,
  error: string | null,
  success: boolean,
...
export const addSponsor = createAsyncThunk("addSponsor", async (row:club_sponsor, api) => {
  try {
    // const config = {
    //   headers: {
...
export const sponsorClubSlice = createSlice({
  name: "sponsorClub",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addSponsor.fulfilled, (state, action) => {
        state.loading = false;
          state.game = action.payload as club_sponsor;
          state.success = true;
          state.error = null;
      })
...
export default sponsorClubSlice.reducer;
