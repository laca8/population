...
import sponsorService from "../services/sponsor/sportSponsor";
import { objectId, sport_sponsor } from "../../types/type";
type obj = sport_sponsor & objectId
const initialState: {
  game: sport_sponsor | null,
  games: sport_sponsor[],
  loading: boolean,
  error: string | null,
  success: boolean,
...
export const addSponsor = createAsyncThunk("addSponsor", async (row:sport_sponsor, api) => {
  try {
    // const config = {
    //   headers: {
...
export const sponsorSportSlice = createSlice({
  name: "sponsorSport",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addSponsor.fulfilled, (state, action) => {
        state.loading = false;
          state.game = action.payload as sport_sponsor;
          state.success = true;
          state.error = null;
      })
...
export default sponsorSportSlice.reducer;
