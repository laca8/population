...
import sponsorService from "../services/sponsor/playerSponsor";
import { objectId, player_sponsor } from "../../types/type";
type obj = player_sponsor & objectId
const initialState: {
  game: player_sponsor | null,
  games: player_sponsor[],
  loading: boolean,
  error: string | null,
  success: boolean,
} = {
...
export const addSponsor = createAsyncThunk("addSponsor", async (row:player_sponsor, api) => {
  try {
    // const config = {
    //   headers: {
...
export const fetchSponsors = createAsyncThunk("fetchSponsors", async (keyword:{name:string,code:string}, api) => {
    try {
      ...
      return games as player_sponsor[];
    } catch (error) {
      // console.log(error);
      return api.rejectWithValue((error as { response: { data: { message: string } } })?.response?.data?.message);
...
export const sponsorPlayerSlice = createSlice({
  name: "sponsorPlayer",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addSponsor.fulfilled, (state, action) => {
        state.loading = false;
          state.game = action.payload as player_sponsor;
          state.success = true;
          state.error = null;
      })
...
export default sponsorPlayerSlice.reducer;
