...
import { player_sponsor, objectId, sport, player } from '../../../types/type';
import { useDispatch, useSelector } from 'react-redux';

import Loader from '../../features/Loader';
import { AppDispatch } from '../../../redux/store';
import { toast } from "react-toastify";
import { addSponsor, editSponsor } from '../../../redux/slicers/playerSponsor'
...
const Add = ({ row }: Props) => {
    ...
    const playerSponsorSlice = useSelector((state: { playerSponsorSlice: { loading: boolean, game: player_sponsor, error: string, success: boolean, games: player_sponsor[] } }) => state.playerSponsorSlice)
    const { loading, error, success } = playerSponsorSlice
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault()
        if (id === '' || name === '' || job === '' || manager === '' || address1 === '' || address2 === '' || phone1 === '' || phone2 === '' || notes === '' || game === '' || player === '') {
        ...
        const sponsorData: player_sponsor = {
            code: Number(id),
            player,
            name: name,
            game,
    ...
    return (
        loading ? <Loader /> : (
            <div >
                ...
                <div className='flex flex-col items-center text-zinc-900 border-2 border-yellow-600 p-2 text-center rounded-xl w-48 m-auto text-xl font-bold mb-4'>
                    <div className="flex gap-8 items-center">
                        <div className="animate-bounce">
                            <LoaderPinwheel className="w-10 h-10 text-yellow-600 shadow-lg" />
                        </div>
                        <h1 className='text-white text-center '>Pl<span className='text-yellow-500 '>a</span>yer_S<span className='text-yellow-500 '>p</span>onsor</h1>
                        <div className="animate-bounce">
