...
import { removeSponsor, fetchSponsors } from '../../../redux/slicers/playerSponsor';
...
const Details = ({ row, setRow, name, code }: Props) => {
    const clubSponsorSlice = useSelector((state: { clubSponsorSlice: { loading: boolean, success: boolean, game: club_sponsor, error: string, games: obj[] } }) => state.clubSponsorSlice)
    const { loading, error, games, success } = clubSponsorSlice
                    ...
                    <tbody>
                        {
                            games?.map((x, i) => (
                                <tr className='hover:bg-zinc-900' key={i} onClick={() => handleClick(x._id)}>
                                    ...
                                    <td className='text-md'>{x.club}</td>
