...
import { addSponsor, editSponsor } from '../../../redux/slicers/clubSponsor'
...
const Add = ({ row }: Props) => {
    ...
    const clubSponsorSlice = useSelector((state: { clubSponsorSlice: { loading: boolean, game: club_sponsor, error: string, success: boolean, games: club_sponsor[] } }) => state.clubSponsorSlice)
    const { loading, error, success } = clubSponsorSlice
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault()
        if (id === '' || name === '' || job === '' || manager === '' || address1 === '' || address2 === '' || phone1 === '' || phone2 === '' || notes === '' || club === '') {
        ...
        const sponsorData: club_sponsor = {
            code: Number(id),
            club,
            name: name,
                ...
                <div >
                    ...
                    <div className='flex flex-col items-center text-zinc-900 border-2 border-yellow-600 p-2 text-center rounded-xl w-40 m-auto text-xl font-bold mb-4'>
                        <div className="flex gap-8 items-center">
                            ...
                            <h1 className='text-white text-center '>Cl<span className='text-yellow-500 '>u</span>b_S<span className='text-yellow-500 '>p</span>onsor</h1>
                            <div className="animate-bounce">
                    ...
                    <form className='w-full ' onSubmit={handleSubmit}>
                        <div className='border-2 border-yellow-600 p-4  shadow-md rounded-xl'>
                            ...
                            <div className='grid grid-cols-4 max-lg:grid-cols-1 justify-center  w-full gap-2 '>
                                ...
                                <div className='flex gap-2 flex-col '>
                                    <label className='font-bold text-zinc-100 bg-yellow-600 p-1 w-20 rounded-xl text-center'>اسم النادي</label>
                                    <input value={club} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setClub(e.target.value)} type="text" placeholder="Type here" className="p-2 bg-gray-50 rounded-xl w-full max-w-xs shadow-md border-2 border-yellow-600" />
